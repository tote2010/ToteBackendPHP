...
class DataTable {
    ...
    public function __construct($model, $name = null) {
        ...
        $this->_model = $model;
        $this->_nombre_tabla = $name ? 'Listado de ' . $name : 'Listado';
    }
    ...
    public function render($attrs = NULL) {
        ...
        $table .= '         <h1 class="page-header">Tables</h1>';
        $table .= '     <div>';
        $table .= '     <!-- /.col-lg-12 -->';
        $table .= ' <div>';
        $table .= ' <!-- /.row -->';
        */
        ...
        $table .= '             <div class="panel-heading">' . $this->_nombre_tabla;
        $table .= '             </div>';
        $table .= '                 <div class="panel-body">';
        $table .= '                     <div class="dataTable_wrapper">';
        $table .= '                         <table class="table table-striped table-bordered table-hover" id="dataTables-' . $this->_id_table . '">';
     ...
     * condicionar la url/texto a utilizar en el link (opcional)
     */
    public function cssLink($attr, $action, $text = NULL, $boolean_field = NULL) {
        //$url_img = explode('|', $url_img);
        $attr = explode('|', $attr);
        $action = explode('|', $action);
        $text = explode('|', $text);
        //isset($url_img[1]) || $url_img[1] = $url_img[0];
        isset($attr[1]) || $attr[1] = $attr[0];
        isset($action[1]) || $action[1] = $action[0];
        isset($text[1]) || $text[1] = $text[0];
        $this->addFields(array(
            'field' => $this->_primary_key,
            'boolean_field' => $boolean_field,
            'options' => array(
                Html::link("$action[0]/%s", "$text[0]", $attr[0]),
                Html::link("$action[1]/%s", "$text[1]", $attr[1]),
            )
        ));
    }
     ...
     * condicionar la url/texto a utilizar en el link (opcional)
     */
    public function cssLinkConfirm($class, $attr, $action, $text, $confirm = '¿Esta Seguro?', $boolean_field = FALSE) {
        $attr = explode('|', $attr);
        $action = explode('|', $action);
        $text = explode('|', $text);
        ...
        $this->addFields(array(
            ...
            'boolean_field' => $boolean_field,
            'options' => array(
                Js::link("$action[0]/%s", $text[0], $confirm[0], $class, $attr[0]),
                // VER ERROR: Notice: Undefined offset: 1 in /opt/lampp/htdocs/petapp/default/app/libs/data_table.php on line 422
                //Js::link("$action[1]/%s", $text[1], $confirm[1], $class, $attr[1]),
            )
        ));
    }
    ...
    /**
     * Establece el id para para la tabla, si no se estable usa el
     * backend.
     *
     * Ejemplo:
     *
    ...
    protected function _paginator() {
        ...
        if (!$this->_type_paginator) {
            ...
            $html .= '<div class="dataTables_info" id="dataTables-example_info" aria-live="polite">';
            $html .= '<b>Total registros: ' . $this->_paginator->count . '</b>';
            $html .= '</div>';
            $html .= '</div>';
            
            $html .= '<div class="col-sm-6">';
            $html .= '<div class="dataTables_paginate paging_simple_numbers" id="dataTables-example_paginate">';       
            $html .= '<ul class="pagination">';
            
            if ($this->_paginator->count > $this->_paginator->per_page) {
                if ($this->_paginator->prev) {
                    $html .= '<li class="paginate_button previous" aria-controls="dataTables-example" tabindex="0" id="dataTables-example_previous">';
                    ...
                    $html .= Html::link('#', 'Anterior', 'title="Ir a la p&aacute;g. anterior" style="pointer-events: none;"');
                    $html .= '</li>';
                }
                for ($x = 1; $x <= $this->_paginator->total; ++$x) {
                    //$html .= '<li class="paginate_button" aria-controls="dataTables-example" tabindex="0">';
                    $html .= $this->_paginator->current == $x ? '<li class="paginate_button active" aria-controls="dataTables-example" tabindex="0">' . Html::link($this->_url . $x, $x, ' style="pointer-events: none;"') . '</li>' : '<li class="paginate_button" aria-controls="dataTables-example" tabindex="0">' . Html::link($this->_url . $x, $x) . '</li>';
                    //$html .= '</li>';
                    //$html .= '&nbsp;&nbsp;';
                }
                if ($this->_paginator->next) {
                    $html .= '<li class="paginate_button next" aria-controls="dataTables-example" tabindex="0" id="dataTables-example_previous">';
                    $html .= Html::link($this->_url . $this->_paginator->next, 'Siguiente', 'title="Ir a la p&aacute;g. siguiente"');
                    $html .= '</li>';   
                } else {
                    $html .= '<li class="paginate_button next disabled" aria-controls="dataTables-example" tabindex="0" id="dataTables-example_previous">';
                    $html .= Html::link('#', 'Siguiente', 'title="Ir a la p&aacute;g. siguiente" style="pointer-events: none;"');
                    $html .= '</li>';  
    ...
    protected function _paginators() {
        if (!$this->_url) {
            if (Router::get('module'))
                $this->_url = Router::get('module') . '/' . Router::get('controller') . '/' . Router::get('action') . '/';
            else
