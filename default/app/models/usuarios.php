...
//put your code here
  //public $debug = true;
    ...
    /**
     * Devuelve los usuarios del sistema de la bd Paginados.
     * 
     * @return array        resultado de la consulta
     */
    public function paginarUsuariosSistema($pagina = 1)
    {
        $sql = 'SELECT  cli.*, 
                        rolUsu.usuarios_id as rolUsu,
                        rol.id as rol
                from usuarios cli 
                INNER JOIN roles_usuarios rolUsu ON cli.id = rolUsu.usuarios_id
                INNER JOIN roles rol ON rolUsu.roles_id = rol.id
                WHERE rol.id = 1
                   OR rol.id = 2';
    ...
    /**
     * Devuelve un arreglo con los usuarios del sistema.
     * 
     * @return array          resultado de la consulta
     */
    public function listarUsuariosSistema()
    {
        $sql = 'SELECT DISTINCT usr.*, 
                        rolUsu.usuarios_id as rolUsu
                from usuarios usr 
                INNER JOIN roles_usuarios rolUsu ON usr.id = rolUsu.usuarios_id
                WHERE rolUsu.roles_id = 1
                   OR rolUsu.roles_id = 2';

        return $this->find_all_by_sql("$sql");
    }
    
    /**
     * Devuelve un arreglo con los usuarios cliente del sistema.
     * 
     * @return array          resultado de la consulta
     */
    public function listarClientesSistema()
    {
        $sql = 'SELECT DISTINCT usr.*, 
                        rolUsu.usuarios_id as rolUsu
                from usuarios usr 
                INNER JOIN roles_usuarios rolUsu ON usr.id = rolUsu.usuarios_id
                WHERE rolUsu.roles_id = 3';
    ...
    public function guardar($data, $roles)
    {
        ...
        $this->clave2 = $data["clave2"];
        
        $rolUser = Load::model('roles_usuarios');

        if ($roles) {
            if (!$this->save($data) || !$rolUser->asignarRol($this->id, $roles)) {
     ...
     *
     * @param array $data datos que se enviaron del form
     * @param $roles id del rol a guardar para el user
     * @return boolean retorna TRUE si se pudieron guardar los datos con exito
     */
    public function guardarGestionador($data, $roles)
    {
        $this->begin();
    ...
    public function actualizar($data)
    {
        ...
        if ($roles) {
                ...
                if (!$this->update($data) || !$rolUser->actualizarRol($this->id, $roles)) {
                    Flash::error('No se pudieron Guardar los Roles para el usuario');
                    $this->rollback();
                    return FALSE;
                }
    ...
    /**
     * Retorna el id del rol del usuario.
     * 
     * @param int $usuario->id
     * @return String
     */
    public function rolIdUsuario($id)
    {
        $rol_usuario = Load::model('roles_usuarios')->find_first("usuarios_id = $id");
        return $rol_usuario->roles_id;
        /*
        $roles_id = array();
    ...
    {
        $res = //Load::model('roles')->find_first('select plantilla 
				Load::model('roles')->find_by_sql('select plantilla,MAX(c)
